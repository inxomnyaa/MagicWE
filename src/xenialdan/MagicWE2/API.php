...
class API
{
    ...
    {
        if (empty($oldBlocks)) {
            $session->sendMessage(TF::RED . "Old blocks is empty!");
        }
        if (empty($newBlocks)) {
            $session->sendMessage(TF::RED . "New blocks is empty!");
        }
    ...
    public static function createBrush(Block $target, Brush $brush, Session $session): void
    {
        ...
        //TODO remove hack
        if ($actionClass === SetBiomeAction::class) {
            $brush->properties->actionProperties["biomeId"] = $brush->properties->biomeId;
        }
    ...
    { //TODO
        if (!$settings instanceof CompoundTag) {
            return false;
        }
    ...
    public static function evalAsMath(string $str)
    {
        ...
        if ($error) {
            throw new CalculationException("Expression contains an error");
        }
