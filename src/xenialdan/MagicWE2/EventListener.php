...
//	}
	...
	private function onRightClickBlock(PlayerInteractEvent $event) : void{
		...
		$item = $event->getItem();
		if(!is_null($item->getNamedTag()->getCompoundTag(API::TAG_MAGIC_WE)) || !is_null($item->getNamedTag()->getCompoundTag(API::TAG_MAGIC_WE_ASSET)) || !is_null($item->getNamedTag()->getCompoundTag(API::TAG_MAGIC_WE_DEBUG))){
			$event->cancel();
			$session = SessionHelper::getUserSession($event->getPlayer());
			if(!$session instanceof UserSession) return;
			switch($item->getId()){
				...
				{
					#if (){// && has perms
					API::floodArea($event->getBlock()->getSide($event->getFace()), $item->getNamedTag()->getCompoundTag(API::TAG_MAGIC_WE), $session);
					#}
					break;
				}
				case ItemIds::SCAFFOLDING:
				{
					$tag = $item->getNamedTag()->getCompoundTag(API::TAG_MAGIC_WE_ASSET);
					if($tag !== null){
	...
	private function onRightClickAir(PlayerItemUseEvent $event) : void{
		if(!is_null(($item = $event->getItem())->getNamedTag()->getCompoundTag(API::TAG_MAGIC_WE_BRUSH))){
			$event->cancel();
			$session = SessionHelper::getUserSession($event->getPlayer());
			if(!$session instanceof UserSession) return;
