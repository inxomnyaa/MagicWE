...
class UserSession extends Session implements JsonSerializable //TODO use JsonMapper
{
    ...
    public function setSidebarEnabled(bool $sidebarEnabled): string
    {
        ...
        if (!$player instanceof Player) {
            return TF::RED . "Session has no player";
        }
    ...
    public function removeBrush(Brush $brush, bool $delete = false): void
    {
        ...
        if ($delete) {
            $this->sendMessage($this->getLanguage()->translateString('session.brush.deleted', [$brush->getName(), $brush->properties->uuid]));
        } else {
            $this->sendMessage($this->getLanguage()->translateString('session.brush.removed', [$brush->getName(), $brush->properties->uuid]));
        }
    }
    ...
    {
        file_put_contents(
            Loader::getInstance()->getDataFolder() . "sessions" . DIRECTORY_SEPARATOR .
