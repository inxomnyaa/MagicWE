...
class BlockStatesEntry
{
    ...
    {
        if (strpos($blockIdentifier, "_door") !== false) {
            var_dump($blockIdentifier);
        }
        ...
        foreach ($bsCompound as $tagName => $tag) {
            ...
            if ($axis === FlipAction::AXIS_Y) {
                ...
                switch ($tagName) {//TODO clean up oh my god
                    ...
                    {
                        if ($value === "down_east_west") {
                            $value = "up_east_west";
                        } elseif ($value === "up_east_west") {
                            $value = "down_east_west";
                        } elseif ($value === "down_north_south") {
                            $value = "up_north_south";
                        } elseif ($value === "up_north_south") {
            ...
            } elseif (isset($flippedStates)) {
                $value = $flippedStates[$tagName] ?? $tag->getValue();
            } else {
                throw new InvalidArgumentException("flippedStates is not set. Error should never occur, please use //report and send a stack trace");
            }
            if ($tag instanceof StringTag) {
