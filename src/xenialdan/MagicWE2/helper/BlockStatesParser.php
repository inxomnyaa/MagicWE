...
final class BlockStatesParser
{
    ...
    private function loadLegacyMappings(): void
    {
        ...
        if ($contents === false) {
            throw new PluginException("Can not get contents of r12_to_current_block_map");
        }
    ...
    public static function fromString(string $query, bool $multiple = false): array
    {
        ...
        if ($multiple) {
            ...
            if (!is_array($pregSplit)) {
                throw new InvalidArgumentException("Regex matching failed");
            }
        ...
        foreach ($statesExploded as $stateKeyValuePair) {
            if (strpos($stateKeyValuePair, "=") === false) {
                continue;
            }
        ...
        if (empty($blocks)) {
            throw new InvalidArgumentException("No block $namespacedSelectedBlockName matching $query could be found");
        }//no block found //TODO r12 map only has blocks up to id 255. On 4.0.0, return Item::fromString()?
        if (count($blocks) === 1) {
            return $blocks;
        }
    ...
    public static function getStateByCompound(CompoundTag $compoundTag): ?BlockStatesEntry
    {
        ...
        if ($namespacedSelectedBlockName === null) {
            return null;
        }
    ...
    public static function printStates(BlockStatesEntry $entry, bool $skipDefaults): string
    {
        ...
        foreach ($printedCompound as $statesTagEntryName => $statesTagEntry) {
            ...
            if ($skipDefaults && $namedTag->equals($statesTagEntry)) {
                continue;
            }
            //prepare string
            ...
            } elseif ($statesTagEntry instanceof IntTag) {
                $s[] = TF::BLUE . $statesTagEntryName . "=" . TF::BLUE . $statesTagEntry->getValue() . TF::RESET;
            } elseif ($statesTagEntry instanceof StringTag) {
