		...
		if ($session->isSidebarEnabled()) {
			...
			try {
				if ($session->getLatestSelection() !== null) {
					...
					ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "Position: " . TF::RESET . API::vecToString($selection->getPos1()->asVector3()) . " » " . API::vecToString($selection->getPos2()->asVector3()));
					...
					ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "Size: " . TF::RESET . API::vecToString(new Vector3($selection->getSizeX(), $selection->getSizeY(), $selection->getSizeZ())) . " ({$selection->getShape()->getTotalCount()})");
					...
					ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "Limit: " . TF::RESET . ($editLimit === -1 ? API::boolToString(false) : $editLimit));
					ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "Wand Tool: " . TF::RESET . API::boolToString($session->isWandEnabled()));
					ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "Debug Tool: " . TF::RESET . API::boolToString($session->isDebugToolEnabled()));
					ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "WAILA: " . TF::RESET . API::boolToString($session->isWailaEnabled()));
					...
					if (($cb = $session->getCurrentClipboard()) instanceof SingleClipboard) {
						...
						if ($cb->selection instanceof Selection) {
							...
							ScoreFactory::setScoreLine($player, ++$line, TF::ITALIC . "Size: " . TF::RESET . API::vecToString(new Vector3($cb->selection->getSizeX(), $cb->selection->getSizeY(), $cb->selection->getSizeZ())) . " ({$cb->getTotalCount()})");
						}
					}
